...
#   self-assessment tool and prenatal health-behavior scores:
#   a real-world cohort study of BabyQ users"
#
...
# ============================================================
# RESULTS — Minimum Detectable Effect (MDE) at fixed N per arm
# ============================================================
n_fixed <- c(100, 200, 400, 800)
cat("Minimum detectable effect (alpha=0.05, power=80%):\n")
for (n in n_fixed) {
  mde <- (z_alpha + z_beta) * sd_diff * sqrt(2 / n)
  cat("  N =", n, "per arm: MDE =", round(mde, 1), "points\n")
}
...
# ============================================================
# SENSITIVITY ANALYSIS 2 — Permutation Test: Continuous Exposure
# ============================================================
n_perm <- 5000
perm_betas_cont <- numeric(n_perm)

for (i in 1:n_perm) {
  fl_perm <- fl %>% mutate(n_surveys_perm = sample(n_surveys))
  perm_betas_cont[i] <- coef(lm(diff ~ n_surveys_perm, data = fl_perm))["n_surveys_perm"]
}

observed_beta_cont <- coef(lm(diff ~ n_surveys, data = fl))["n_surveys"]
perm_p_cont <- mean(abs(perm_betas_cont) >= abs(observed_beta_cont))

cat("=== Permutation test: continuous exposure (n_surveys) ===\n")
cat("Observed beta:", round(observed_beta_cont, 3), "\n")
cat("Permutation null mean:", round(mean(perm_betas_cont), 3), "\n")
cat("Permutation null SD:", round(sd(perm_betas_cont), 3), "\n")
cat("Permutation P-value:", perm_p_cont, "\n")
...
# ============================================================
# TABLE 1 — Per-band descriptive statistics
# ============================================================
table1_stats <- fl %>%
  group_by(band) %>%
  summarise(
    n               = n(),
    mean_first      = round(mean(first_score, na.rm = TRUE), 1),
    sd_first        = round(sd(first_score, na.rm = TRUE), 1),
    mean_last       = round(mean(last_score, na.rm = TRUE), 1),
    sd_last         = round(sd(last_score, na.rm = TRUE), 1),
    mean_delta      = round(mean(diff, na.rm = TRUE), 2),
    pct_improvement = round(mean(diff, na.rm = TRUE) / mean(first_score, na.rm = TRUE) * 100, 1)
  )
cat("=== Table 1: Per-band descriptive statistics ===\n")
print(table1_stats)
...
# ============================================================
# (For Table 2) RESULTS — Longitudinal Cluster-Robust (CR1) Pooled-OLS Model
# ============================================================
# install.packages("clubSandwich")
library(clubSandwich)

df_long_cr1 <- df_long %>%
  left_join(fl %>% select(User, band_ordinal), by = "User") %>%
  mutate(calendar_time = as.numeric(SurveyDate - min(SurveyDate, na.rm = TRUE)))

cr1_model <- lm(score ~ survey_number + first_score + calendar_time + band_ordinal,
                data = df_long_cr1)

cr1_vcov    <- vcovCR(cr1_model, cluster = df_long_cr1$User, type = "CR1")
cr1_results <- coef_test(cr1_model, vcov = cr1_vcov, test = "z")

cat("=== Longitudinal Pooled-OLS Model, CR1 Cluster-Robust SEs ===\n")
print(cr1_results)


# ============================================================
# TABLE 2 — Assembly: Multivariable dose-response models
# ============================================================

# Extract cluster-robust SEs directly from the CR1 covariance matrix
# (avoids depending on the exact column names in cr1_results' print format)
cr1_se <- sqrt(diag(cr1_vcov))

table2 <- data.frame(
  Model = c("Unadjusted",
            "+ Baseline score",
            "+ Baseline + follow-up days",
            "Longitudinal CR1 (per survey #)"),
  Beta = c(
    round(coef(m1)["n_surveys"], 2),
    round(coef(m2)["n_surveys"], 2),
    round(coef(m3)["n_surveys"], 2),
    round(coef(cr1_model)["survey_number"], 2)
  ),
  CI_low = c(
    round(confint(m1)["n_surveys", 1], 2),
    round(confint(m2)["n_surveys", 1], 2),
    round(confint(m3)["n_surveys", 1], 2),
    round(coef(cr1_model)["survey_number"] - 1.96 * cr1_se["survey_number"], 2)
  ),
  CI_high = c(
    round(confint(m1)["n_surveys", 2], 2),
    round(confint(m2)["n_surveys", 2], 2),
    round(confint(m3)["n_surveys", 2], 2),
    round(coef(cr1_model)["survey_number"] + 1.96 * cr1_se["survey_number"], 2)
  ),
  P = c(
    round(summary(m1)$coefficients["n_surveys", "Pr(>|t|)"], 4),
    round(summary(m2)$coefficients["n_surveys", "Pr(>|t|)"], 4),
    round(summary(m3)$coefficients["n_surveys", "Pr(>|t|)"], 4),
    round(2 * (1 - pnorm(abs(coef(cr1_model)["survey_number"] / cr1_se["survey_number"]))), 4)
  ),
  R2 = c(
    round(summary(m1)$r.squared, 3),
    round(summary(m2)$r.squared, 3),
    round(summary(m3)$r.squared, 3),
    NA
  ),
  N = c("601", "601", "601", "1,907 surveys / 601 users")
)

cat("=== Table 2: Multivariable dose-response models ===\n")
print(table2)
